<template>
  <el-container style="width: 100%;">
    <el-main>
      <el-button style="width: 200px; margin-bottom: 20px;" @click="$router.push('/resource')" >填写周报</el-button>
      <el-table v-loading="loading" :data="list" border row-key="id" style="width: 100%" @row-click="handleRowClick">
        ...
        <el-table-column label="功能区" @click="$router.push('/resource')">
          ...
          <template slot-scope="scope">
            ...
            <el-button @click="goToWeeklyNewspaper(scope.row.subDate)">查看详情</el-button>
            <el-button @click="goTomodificationSubmission(scope.row.subDate)">修改提交</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    goToWeeklyNewspaper(subDate) {
    this.$router.push({ path: '/weeklyNewspaper', query: { subDate: subDate } });},

    goTomodificationSubmission(subDate) {
    this.$router.push({ path: '/modificationSubmission', query: { subDate: subDate}});},
    fetchData() {
      const url = '/admin/mission/checkPdpById';
      // const url = '/admin/pdp/getWeekList';
      const params = { ...this.params, userId: localStorage.getItem("userId"), userName: localStorage.getItem("userName"),missionStatus:"pdp"};// ,missionStatus:"pdp"
      this.loading = true;
      axios
        .post(url, params)
        .then(response => {
